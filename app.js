...
function render(){
 ...
 $("extras").innerHTML=d.extras.map((x,i)=>`<tr><td><input value="${String(x.name??"").replace(/"/g,"&quot;")}" oninput="editExtra(${i},'name',this.value)"></td><td><input type="number" step=".01" min="0" value="${x.qty}" oninput="editExtra(${i},'qty',this.value)"></td><td><input type="number" step=".01" min="0" value="${x.price}" oninput="editExtra(${i},'price',this.value)"></td><td><b>${money((+x.qty||0)*(+x.price||0))}</b></td><td class="no-print"><button class="remove" onclick="removeExtra(${i})">Usuń</button></td></tr>`).join("");
